...
func (r *SwimDigitalNotamProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	configHash, certResult, err := r.reconcileProviderAppConfig(ctx, cr, clusterDomain)
	if res, halt, e := shouldHaltAfterProviderStepEmptyOnError(certResult, err); halt {
		return res, e
	}
	...
	r.updateProviderStatus(ctx, cr, "Available", metav1.ConditionTrue, "Reconciled", "All resources created successfully")
	return ctrl.Result{}, nil
}
